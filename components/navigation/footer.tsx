...
const socialMedia = [
  ...
  { name: 'Instagram', icon: 'instagram', href: 'https://instagram.com/mfaudio' },
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 border-b border-gray-800 pb-12">
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <a href={`mailto:${companyData.email}`} className="text-gray-400 hover:text-white transition-colors">{companyData.email}</a>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          <div className="flex flex-col justify-between">
            <div>
              <Image src="/placeholder.svg" alt="MF Audio Logo" width={180} height={60} className="mb-6" />
              <p className="text-gray-400 mb-6">Elevating your audio experience with cutting-edge technology and unparalleled sound quality.</p>
            </div>
            <div>
              <h3 className="text-lg font-semibold mb-4">Find us on:</h3>
              <div className="flex space-x-4">
                {socialMedia.map((item) => (
                  <Link key={item.name} href={item.href} aria-label={item.name} className="text-gray-400 hover:text-white transition-colors">
                    {item.icon === 'tiktok' ? (
          ...
          <div className="grid grid-cols-2 gap-8">
            <div>
              ...
              <ul className="space-y-2">
                {customerZone.map((item) => (
                  <li key={item.name}>
                    {item.download ? (
                      <Link href={item.href} download className="text-gray-400 hover:text-white transition-colors inline-flex items-center">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {models.map((model) => (
                  <li key={model}>
                    <Link href={`/models/${model.toLowerCase().replace(' ', '-')}`} className="text-gray-400 hover:text-white transition-colors">
                      {model}
                    </Link>
                  </li>
                ))}
